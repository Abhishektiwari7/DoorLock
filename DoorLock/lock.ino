...
const int ledPin = 25;          // The built in LED
...
void setup() {
 ...
 Serial.begin(115200);
  ...
  Lockservo.attach(13);
     // Load the secret knock (if any) from EEPROM.
  doorUnlock(20);     // Unlock the door for a bit when we power up. For system check and to allow a way in if the key is forgotten.
  readSecretKnock();
  delay(100);          // This delay is here because the solenoid lock returning to place can otherwise trigger and inadvertent knock.
}
...
void loop() {
 //EEPROM.read(1);
  ...
  Serial.println(analogRead(knockSensor));
      Serial.println(digitalRead(programButton));
...
void doorUnlock(int delayTime){
  ...
  delay(delayTime);
  digitalWrite(ledPin, LOW);  
  delay(50);   // This delay is here because releasing the latch can cause a vibration that will be sensed as a knock.
  Lockservo.detach();
  delay(4000);
}
